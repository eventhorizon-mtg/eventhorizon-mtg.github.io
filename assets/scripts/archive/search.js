/**
 * Archive Search UI
 * Search placeholder responsive, reset button, and filter popover
 * Espone: window.ArchiveSearch
 */
...
;(() => {
  ...
  const { qs, on, getParam } = window.ArchiveShared
  const { mqPhone, TRUE, FALSE } = window.ArchiveConfig

  // ===========================
  // Search UI (placeholder responsive)
  // ===========================
  ;(() => {
  ...
  // ===========================
  // Search Reset + Filter (popover mobile)
  // ===========================
  ;(() => {
    const form = qs('#archive-search-form')
  ...
  window.ArchiveSearch = {
    // Public API placeholder (currently all handlers are auto-bound)
  }
})()
