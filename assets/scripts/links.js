...
(() => {
  ...
  function initEqualizeGrid(section) {
    ...
    const onResize = () => { clearTimeout(rt); rt = setTimeout(compute, 150); }; // Debounce aumentato per performance

    if (document.readyState === 'complete') compute();
    else window.addEventListener('load', compute, { once: true });
    compute();
    window.addEventListener('resize', onResize, { passive: true });
    section.querySelectorAll('.link-card img').forEach(img => img.addEventListener('load', compute, { passive: true }));
