...
> Nota: questo articolo è un catalogo ESEMPIO COMPLETO di tutte le personalizzazioni che puoi usare nei testi.
...
```javascript
// Questo è un blocco di codice molto lungo che dovrebbe essere scrollabile orizzontalmente su mobile
function esempioFunzioneConParametriMoltiEUnNomeEstremamenteLungoPerTestareLoScrollOrizzontale(parametro1, parametro2, parametro3, parametro4) {
  const variabileMoltoLungaPerTestareLoScrollOrizzontale = "Questo è un valore molto lungo";
...
- Questo è un elemento di lista molto lungo che contiene una descrizione dettagliata per testare come il testo va a capo su dispositivi mobili e se mantiene la corretta indentazione e spaziatura
...
> Questo è un blockquote molto lungo che serve per testare come il testo viene gestito all'interno delle citazioni su diversi viewport. Il blockquote dovrebbe mantenere una buona leggibilità anche su dispositivi mobili piccoli e il testo dovrebbe andare a capo correttamente senza causare overflow orizzontale. Questo è particolarmente importante per citazioni che contengono URL lunghi come <https://www.esempio-url-lungo-in-blockquote.com/path/molto/lunga> o parole tecniche lunghe.
...
**Contenitore HTML personalizzato:** Questo testo è all'interno di un div con stili inline per testare la gestione di HTML custom all'interno degli articoli.
...
[Questo è un link molto lungo verso una risorsa esterna che dovrebbe andare a capo correttamente senza rompere il layout](https://www.example.com/path/molto/lungo/di/test/che/si/prolunga/ancora/di/piu)
{.text-right}
...
- Primo livello
  ...
  - Secondo livello item 2 con testo molto lungo per testare l'indentazione su dispositivi mobili
    - Terzo livello con testo ancora più lungo per verificare la gestione delle liste annidate profonde
...
Di default i pulsanti negli articoli sono **piccoli** per integrarsi meglio con il testo. Per usare pulsanti più grandi e prominenti, aggiungi la classe `.btn-lg` o `.btn--large`:
